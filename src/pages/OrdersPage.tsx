...
import { Search, Package, Truck, CheckCircle, Clock, X, Plus, Trash2 } from 'lucide-react';
...
export default function OrdersPage() {
  ...
  const [branch, setBranch] = useState(branches[0]?.name || 'New York HQ');
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addOrder({
        ...
        branch: branch || branches[0]?.name || 'New York HQ',
        products: validItems,
    ...
    <div className="page-enter">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-0.5 sm:mt-1 text-sm">Track and manage customer orders</p>
        </div>
        {isAuthorizedToAdd && (
          <button
      ...
      <div className="card p-3 sm:p-4 mb-4 sm:mb-6">
        <div className="flex flex-col gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search orders..." value={search} onChange={e => setSearch(e.target.value)} className="form-input pl-8 sm:pl-10" />
          </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredOrders.map(order => (
              <tr key={order.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedOrder(order)}>
                ...
                <td className="table-cell hidden sm:table-cell text-slate-600 text-xs sm:text-sm truncate">{order.customer}</td>
                <td className="table-cell hidden lg:table-cell text-slate-600 text-xs">{order.branch}</td>
                <td className="table-cell text-right font-semibold">£{order.totalAmount.toLocaleString()}</td>
      ...
      {showAddModal && (
        <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="modal-content p-4 sm:p-6 max-w-xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-overlay" onClick={() => setSelectedOrder(null)}>
          <div className="modal-content p-4 sm:p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <div>
                <h2 className="text-lg sm:text-xl font-semibold text-slate-800">{selectedOrder.id}</h2>
                <p className="text-xs sm:text-sm text-slate-500">{selectedOrder.customer}</p>
              </div>
              <button onClick={() => setSelectedOrder(null)} className="p-1.5 sm:p-2 hover:bg-slate-100 rounded-lg"><X size={18} /></button>
            </div>
            <div className="p-3 sm:p-4 bg-slate-50 rounded-lg mb-3 sm:mb-4 text-xs sm:text-sm">
              <div className="flex justify-between"><span className="text-slate-500">Branch</span><span>{selectedOrder.branch}</span></div>
            </div>
            <div className="space-y-2 mb-3 sm:mb-4">
              {selectedOrder.products.map((p, i) => (
                <div key={i} className="flex items-center justify-between p-2 sm:p-3 border rounded-lg">
                  <div><p className="text-xs sm:text-sm font-medium">{p.name}</p><p className="text-xs text-slate-500">{p.quantity} units</p></div>
                  <span className="font-semibold text-xs sm:text-sm">£{p.price}</span>
                </div>
              ))}
            </div>
            ...
            <div className="pt-3 sm:pt-4 border-t mt-3 sm:mt-4 flex justify-between">
              ...
              <span className="text-lg sm:text-xl font-semibold">£{selectedOrder.totalAmount.toLocaleString()}</span>
            </div>
